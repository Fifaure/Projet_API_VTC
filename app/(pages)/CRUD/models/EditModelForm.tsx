...
export default function EditModelForm({ model }: EditModelFormProps) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (data.brand !== model.brand) {
      body.brand = data.brand
    }

    if (data.yearStart !== (model.yearStart?.toString() || '')) {
      body.yearStart = data.yearStart || null
    }

    if (data.yearEnd !== (model.yearEnd?.toString() || '')) {
      body.yearEnd = data.yearEnd || null
    }
    ...
    try {
      ...
      setSuccess(true)
      setTimeout(() => {
        router.push('/CRUD/models/list')
        router.refresh()
      }, 1500)
    } catch (err) {
      console.error('[EditModelForm]', err)
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {success && (
        <div className="p-4 bg-green-50 border border-green-200 rounded-lg text-green-700 text-sm">
          Modèle modifié avec succès ! Redirection en cours...
        </div>
      )}
      ...
      <div className="flex gap-4 pt-4">
        <button
          type="button"
          onClick={() => router.push('/CRUD/models/list')}
        ...
        >
          {isSubmitting ? 'Modification...' : 'Modifier le modèle'}
        </button>
      </div>
    </form>
  )
