...
export async function GET(_: NextRequest, props: Props) {
  try {
    ...
    return NextResponse.json({ data: serializeVehicle(vehicle) })
  } catch {
    return NextResponse.json({ error: 'Impossible de récupérer le véhicule' }, { status: 500 })
  }
}
...
export async function PATCH(request: NextRequest, props: Props) {
  ...
  try {
    ...
    if (body?.modelId !== undefined) {
      ...
      if (!modelId) {
        return NextResponse.json({ error: 'modelId ne peut pas être vide' }, { status: 400 })
      }
    ...
    if (body?.sellerId !== undefined) {
      ...
      if (rawSellerId === null || rawSellerId === '') {
        ...
        if (!sellerId) {
          return NextResponse.json({ error: 'sellerId ne peut pas être vide' }, { status: 400 })
        }
    ...
    if (body?.color !== undefined) {
      data.color = body.color === null ? null : body.color.toString().trim() || null
    }

    if (body?.notes !== undefined) {
      data.notes = body.notes === null ? null : body.notes.toString().trim() || null
    }
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === 'P2003') {
        return NextResponse.json({ error: 'modelId ou sellerId invalide' }, { status: 400 })
      }
      if (error.code === 'P2025') {
        return NextResponse.json({ error: 'Véhicule introuvable' }, { status: 404 })
      }
    }
    return NextResponse.json({ error: 'Impossible de mettre à jour le véhicule' }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest, props: Props) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Impossible de supprimer le véhicule' }, { status: 500 })
  }
}
